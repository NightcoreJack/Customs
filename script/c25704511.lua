...
s.listed_series={0xde8}
function s.filter(c,e,tp,ft) 
	return c:IsMonster() and c:IsSetCard(0xde8) and (c:IsAbleToHand() or (ft>0 and c:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP_DEFENSE)))
end
...
function s.repfilter(c,tp)
	return c:IsFaceup() and c:IsControler(tp) and c:IsLocation(LOCATION_MZONE)
		and c:IsType(TYPE_XYZ) and not c:IsReason(REASON_REPLACE) and c:IsReason(REASON_EFFECT)
end
