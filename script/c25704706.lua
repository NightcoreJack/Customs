...
s.listed_names={25704705}
function s.fcfilter(c)
	return c:IsCode(25704705) and c:IsAbleToHand()
end
function s.tgfilter(c)
	return c:IsType(TYPE_MONSTER) and c:IsSetCard(0x6f9) and c:IsAbleToGrave()
end
function s.mcfilter(c,tp)
	return c:IsType(TYPE_MONSTER) and c:IsSetCard(0x6f9) and c:IsAbleToHand() 
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.fcfilter,tp,LOCATION_DECK,0,1,nil)
			or (Duel.IsExistingMatchingCard(aux.FilterFaceupFunction(Card.IsCode,25704705),tp,LOCATION_FZONE,0,1,nil) 
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local b=Duel.IsExistingMatchingCard(aux.FilterFaceupFunction(Card.IsCode,25704705),tp,LOCATION_FZONE,0,1,nil) 
		and Duel.IsExistingMatchingCard(s.mcfilter,tp,LOCATION_DECK,0,1,nil,tp)
