...
function s.actcostfilter(c,tp,dsp,gg,rg)
	if not c:IsSetCard(0x661) and c:IsAbleToRemoveAsCost() and (c:IsLocation(LOCATION_HAND) or aux.SpElimFilter(c,true)) then return false end
	return (Duel.GetMZoneCount(tp,c)>0 and (dsp or #(gg-c)>0)) or (rg and #(rg-c)>0)
end
function s.spfilter(c,e,tp)
	return c:IsSetCard(0x661) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function s.rmfilter(c,tp)
	return c:IsAbleToRemove() and c:IsFaceup() and c:IsSetCard(0x661)
end
function s.actcost(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	local dsp=Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_DECK,0,1,nil,e,tp,0x661)
	local gg=Duel.GetMatchingGroup(s.spfilter,tp,LOCATION_GRAVE,0,nil,e,tp,0x661)
...
function s.actop(e,tp,eg,ep,ev,re,r,rp)
	...
	if op==1 or op==2 then
		...
		if op==1 then
			g=Duel.SelectMatchingCard(tp,s.spfilter,tp,LOCATION_DECK,0,1,1,nil,e,tp,0x661)
		else
			g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.spfilter),tp,LOCATION_GRAVE,0,1,1,nil,e,tp,0x661)
		end
		if #g>0 then
